 ...
 * @LastEditTime: 2021-04-15 23:11:19
-->
...
> 1. 防抖：在规定的时间内多次触发某一事件，会把之前的回调取消，以最后规定时间内一次的触发为准
>
> - this 指向 window 问题
> - event 指向 undefined 问题
> - 怎么实现立即执行
> - 返回值（只考虑 immediate 为 true 的情况）
> - 取消
...
```JavaScript
  function debounce(fn, wait, immediate){
    ...
    let debounced = function () {
      ...
      return res;//只有在immdiate是true时res才可能有值，false时为undefined
    }
...
> - 定时器、时间戳结合实现（通过 options 参数决定无头有尾、有头无尾）
...
```JavaScript
    ...
    function throttle4 (fn, wait, options) {
      ...
      let throttled =  function () {
        ...
        if (!pre && options.leading === false) pre = curr;//禁用第一次，采用setTimeout

        let remaining = wait - (curr - pre);//禁用第一次时，remaining == wait
        ...
        } else if (!timer && options.trailing !== false) {//禁用第一次时
          timer = setTimeout(function () {
...
- 创建一个对象
- 把构造函数的原型对象赋值给 obj.\_\_proto\_\_
- 为 obj 绑定 this
...
```JavaScript
  Function.prototype.myBind (context) {
    ...
    var fBound = function () {
      //获取new bind返回的函数时所传的参数
      var bindArgs = Array.prototype.slice.call(arguments);
      //修改this
      return self.apply(this instance fNOP ? this : context, args.concat(bindArgs));
    }
...
> 数组的扁平化，就是将一个嵌套多层的数组 array 转换成只有一层的数组
...
- toString（如果数组的元素都是数字，可以考虑这种）

  ```JavaScript
    function flatten (arr) {
      return arr.toString().split(",").map(function (item) {
        return +item;//将每个元素转回Number类型
      })
    }
  ```
...
- 扩展运算符（扩展运算符，用于取出参数对象的所有可遍历属性，拷贝到当前对象之中，一次只能扁平一层）
