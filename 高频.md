 ...
 * @LastEditTime: 2021-04-15 22:28:38
-->
...
- 对象继承？
  ...
  - 1. 原型链
    ...
    > 缺点（1）引用类型值共享（2）子类型在实例化时不能向父类型的构造函数传递参数
  ...
  - 2. 盗用构造函数
    ...
    > 优点（1）相对比原型链继承，盗用构造函数的一个优点是可以向父类构造函数传递参数（2）父类中的引用类型值不会共享；缺点（1）必须在函数中定义方法，因此函数不能重用，子类也不能访问父类原型上定义的方法。
  ...
  - 3. 组合继承
    ...
    ```JavaScript
      ...
      let instance1 = new SubType("suph", 24);
      let instance2 = new SubType("hanipig", 25);
    ...
    > 使用的最多，但并不完美，存在效率问题 => 父类构造函数被调用了两次

  - 4. 原型式继承（Object.create()的实现）

    ```JavaScript
      //obj为需要继承的对象
      function Object (obj) {
        let func = function () {};
        func.prototype = obj;//浅复制，实例都会共享此对象
        return new func();
      }
    ```
  ...
  - 5. 寄生式继承
    ...
    ```JavaScript
      function Object2 (obj)) {
        let newObj = object(obj);//object为 任何可返回对象的方法即可
        newObj.newProperty = "anything";//在旧obj基础上添加任意属性
        return newObj;//即实现了继承旧对象又追加了新增的属性
      }
    ```

  - 6. 寄生组合式继承（在组合继承的基础上少调用了一次父类构造函数）

    ```JavaScript
      function inheritPrototype (child, parent)) {
        let prototype = object(parent.prototype);//object为 任何可返回对象的方法即可
        prototype.constructor = child;//修改prototype的construstor属性
        child.prototype = prototype;//直接重写子类的原型对象
      }
